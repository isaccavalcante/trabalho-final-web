...
<form id="100" action="salvarAtendimento" method="post" enctype="multipart/form-data" autocomplete="off" >	
<input type="hidden" value="${atendimento.ateId}" name="ateId">
...
<select style="width:98%" class="select_lista_fun" name="funId">
  <optgroup label="Funcionários" >
  	<c:forEach var="funcionario" items="${funcionarios}">
  		<c:if test="${funcionario.funId==atendimento.funcionario.funId}">
  			<option value="${funcionario.funId}" selected="selected"> ${funcionario.nome} ${funcionario.sobrenome} </option>
  		</c:if>
...
<select style="width:98%" class="select_lista_cli" name="cliId">
  <optgroup label="Clientes" >
  	...
  	<c:forEach var="cliente" items="${clientes}">
  	  	<c:if test="${cliente.cliId==atendimento.cliente.cliId}">
  			<option value="${cliente.cliId}" selected="selected"> ${cliente.nome} ${cliente.sobrenome} </option>
  		</c:if>
    <option value="${cliente.cliId}"> ${cliente.nome} ${cliente.sobrenome}</option>    
    </c:forEach>
  </optgroup>
</select> <br />
Descrição: <input type="text" name="descricao" value="${atendimento.descricao}" style="width:45%" id="desc"> <br />
Data: <input type="text" name="data" value="${atendimento.data}" style="width:65%" readonly > <br />
Moto: <input type="text" name="moto" value="${atendimento.moto}" style="width:65%" value=""> <br />
Placa:<input type="text" name="placa" value="${atendimento.placa}" style="width:65%" value=""> <br />
Total: <input type="text" style="height:25px;width:50px" name="valorTotal" value="${atendimento.valorTotal}" readonly class="texto_total"> <br />
Pago: &nbsp;&nbsp;&nbsp;<input type="text" style="height:25px;width:50px" value="${atendimento.valorPago}" name="valorPago" class="texto_pago"> <br />
Débito: <input type="text" style="height:25px;width:50px" name="debito" value="${atendimento.debito}" readonly class="texto_debito"> <br />
			...
			<td >
				<select style="width:100%" data-index="${i}" class="select_lista_prod" name="prods" id="testeisac">
				  <optgroup label="Produtos">
				  	<option value="${atendimento.produtos.toArray()[(i-1)].precoVenda}#${atendimento.produtos.toArray()[(i-1)].id}" selected="selected"> ${atendimento.produtos.toArray()[(i-1)].nome} </option>
  				  	<c:forEach var="produto" items="${produtos}">
				    	<option value="${produto.precoVenda}#${produto.id}"> ${produto.id} — ${produto.nome} ${produto.referencia} ${produto.codigo}</option>
				    </c:forEach>
				  	</optgroup>
				</select>
			</td>
...
<div align=center><a href="editarAtendimentoFormulario?id=${atendimento.ateId}&linhas=${param.linhas+1}">Adicionar linha </a></div>
<br/>
...
function Imprimir() {
	...
	$(document).ready(function() {
	  ...
	  $(".select_lista_prod").change(function(e){
		   for (i = 1 ;i <= "${param.linhas}"; i++){
		    	if (isNaN($(".total[data-index="+i+"]").text())){
		    		$(".total[data-index="+i+"]").text('');
		    		$(".quantidade[data-index="+i+"]").val('');
		    	}
		    }
	 ...
	 $(".select_lista_prod").trigger('change');
